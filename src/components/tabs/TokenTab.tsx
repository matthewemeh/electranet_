          ...
          <TableCell key={id} role='cell' align={align} style={{ minWidth, maxWidth }}>
            {id === 'email' ? (
              <Link
                ...
                to={`mailto:${value}?subject=Electranet&body=Hello%20${token.user.lastName}%20${token.user.firstName}`}
              >
                {value}
              </Link>
            ) : id === 'rightsStatus' ? (
            ...
            <div className='form mt-4 !mb-0'>
              ...
              <FormGroup>
                <FormControlLabel
                  ...
                  label='Can Expire'
                  control={
                    <Switch checked={canExpire} onChange={e => setCanExpire(e.target.checked)} />
                  }
                />
                <TextField
                  type='text'
